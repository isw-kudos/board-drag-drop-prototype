...
/*global angular: false, Card */

'use strict';

angular.module('boards')

.controller('BoardController', ['$scope','$document','$window','BoardService','$ionicScrollDelegate','$timeout','BoardSize','$ionicModal',
function ($scope, $document, $window, BoardService, $ionicScrollDelegate, $timeout, BoardSize, $ionicModal) {
  ...
  function calculateListWidth(event) {
    var screenWidth = $document[0].body.clientWidth - (2*$scope.boardSize.listOffset);
  ...
  $scope.listSortOptions = {
    ...
    dragMove:function(event){
      ...
      function getScrollInfo(distToPosLimit,distToNegLimit,options) {
        ...
        var distance = (distToPosLimit < options.tollerance) ?options.tollerance-distToPosLimit :(distToNegLimit<options.tollerance) ?distToNegLimit-options.tollerance :0;
        return {
          distance:distance,
      ...
      var distToRightEdge = $document[0].body.clientWidth - distToLeftEdge;
  ...
  $scope.startMoving = function(scroller,pixels,event) {
    function moveBy(pixels) {
      ...
      $timeout(function() {
        var e = $document[0].createEvent('MouseEvent');
        e.initMouseEvent("mousemove",true,true,$window,1,null,null,event.nowX, event.nowY, false, false, false, false, 0, undefined);
